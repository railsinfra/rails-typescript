...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['RAILS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['RAILS_LOG'] or 'warn' if it isn't set.
   */
...
export class Rails {
   ...
   */
  constructor({
    baseURL = readEnv('RAILS_BASE_URL'),
    apiKey = readEnv('RAILS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.RailsError(
        "The RAILS_API_KEY environment variable is missing or empty; either provide it, or instantiate the Rails client with an apiKey option, like new Rails({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('RAILS_LOG'), "process.env['RAILS_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('RAILS_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static RailsError = Errors.RailsError;
